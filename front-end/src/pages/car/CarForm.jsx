  ...
  */
  ...
  async function handleFormSubmit(event) {
    ...
    try {
      ...
      if(carData.selling_price === '' || carData.selling_price === null || carData.selling_price === undefined) {
        carData.selling_price = null
      } else {
        carData.selling_price = Number(carData.selling_price)
      }

      if(carData.year_manufacture) {
        carData.year_manufacture = Number(carData.year_manufacture)
      }
      ...
      else if(error?.status === 422 && error?.body && Array.isArray(error.body)) {
        const errorMessages = {}
        for(let i of error.body) errorMessages[i.path[0]] = i.message
        setState({ ...state, inputErrors: errorMessages })
        notify('Há campos com valores inválidos. Verifique.', 'error')
      }
  ...
  async function loadData() {
    ...
    try {
      ...
      if(params.id) {
        ...
        // Garante que imported seja boolean
        car.imported = Boolean(car.imported)
      }
